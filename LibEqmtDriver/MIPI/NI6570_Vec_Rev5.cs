...
namespace LibEqmtDriver.MIPI
{
    ...
    public class NI_PXIe6570 : iMiPiCtrl
    {
        ...
        bool iMiPiCtrl.SendVector(string nameInMemory)
        {
            return myMipiCtrl.SendVector(nameInMemory);
        }
        int iMiPiCtrl.SendVector_VerifyLNA_OTP(string nameInMemory)
        {
            return myMipiCtrl.SendVector_VerifyLNA_OTP(nameInMemory);
        }
        int iMiPiCtrl.ReadVector(ref int DecRead, string nameInMemory)
        {
            return myMipiCtrl.ReadVector(ref DecRead, nameInMemory);
        }
        bool iMiPiCtrl.LoadVector_PowerMode(string fullPath, string nameInMemory)
        {
            return myMipiCtrl.LoadVector(new List<string>{fullPath}, nameInMemory, false);  //  default is not to datalog number of bit errors
        }
        bool iMiPiCtrl.LoadVector_PowerMode(string fullPath, string nameInMemory, bool ReadFlag, bool ReturnCall)
        {
            return myMipiCtrl.LoadVector(new List<string> {fullPath}, nameInMemory, false, ReadFlag, ReturnCall);    
        }
        ...
        public s_MIPI_DCSet[] mipi_DCSet;
        public s_MIPI_DCMeas[] mipi_DCMeas;
    }
    ...
    public static class HSDIO
    {
        ...
        public class NI6570 : iHsdioInstrument
        {
            ...
            private string allRffeChans, SdataChanName_TX, SdataChanName_RX;
            private DigitalPinSet allRffePins, sdataPins, sdataPin, sdataPin_RX, sclkPins, vioPins, trigPin;
            ...
            public static int Marker_Count = 0;
            ...
            public NI6570(string visaAlias, bool AutoSubCal, s_assignMIPIpin[] mipiCfg)
            {
                ...
                MIPIClockRate = 52e6;//26e6;  // This is the Non-Return to Zero rate, Actual Vector rate is 1/2 of this.                
                ...
                if (allMipiPinNames.Length == 2)
                {
                    SdataChanName_TX = allMipiPinNames[0].SdataPinName;
                    SdataChanName_RX = allMipiPinNames[1].SdataPinName;
                    sdataPin = DIGI.PinAndChannelMap.GetPinSet(SdataChanName_TX);
                    sdataPin_RX = DIGI.PinAndChannelMap.GetPinSet(SdataChanName_RX);
                }                
                ...
                // Create Timeset
                DigitalTimeSet tsNRZ = DIGI.Timing.CreateTimeSet(Timeset.MIPI_SCLK_NRZ.ToString("g"));
                ...
                // Standard .vec files use the Non Return to Zero Format Half Rate
                ...
                // Create Timeset
                DigitalTimeSet tsNRZ_half = DIGI.Timing.CreateTimeSet(Timeset.MIPI_SCLK_NRZ_HALF.ToString("g"));
                tsNRZ_half.ConfigurePeriod(period);

                // Vio, Sdata, Trig
                tsNRZ_half.ConfigureDriveEdges(vioPins, DriveFormat.NonReturn, driveOn, driveData, driveReturn, driveOff);
                tsNRZ_half.ConfigureCompareEdgesStrobe(vioPins, compareStrobe);
                tsNRZ_half.ConfigureDriveEdges(sdataPins, DriveFormat.NonReturn, driveOn, driveData, driveReturn, driveOff);
                tsNRZ_half.ConfigureCompareEdgesStrobe(sdataPins, compareStrobe);
                tsNRZ_half.ConfigureDriveEdges(trigPin, DriveFormat.NonReturn, driveOn, driveData, driveReturn, driveOff);
                tsNRZ_half.ConfigureCompareEdgesStrobe(trigPin, compareStrobe);
                // Sclk
                tsNRZ_half.ConfigureDriveEdges(sclkPins, DriveFormat.NonReturn, driveOn + clockRisingEdgeDelay, driveData + clockRisingEdgeDelay, driveReturn + clockFallingEdgeDelay, driveOff + clockFallingEdgeDelay);
                tsNRZ_half.ConfigureCompareEdgesStrobe(sclkPins, compareStrobe);
                ...
                // Create Timeset
                DigitalTimeSet tsMIPI = DIGI.Timing.CreateTimeSet(Timeset.MIPI.ToString("g"));
                tsMIPI.ConfigurePeriod(period);


                // Vio, Sdata, Trig
                tsMIPI.ConfigureDriveEdges(vioPins, DriveFormat.NonReturn, driveOn, driveData, driveReturn, driveOff);
                tsMIPI.ConfigureCompareEdgesStrobe(vioPins, compareStrobe);
                tsMIPI.ConfigureDriveEdges(sdataPins, DriveFormat.NonReturn, driveOn, driveData, driveReturn, driveOff);
                tsMIPI.ConfigureCompareEdgesStrobe(sdataPins, compareStrobe);
                tsMIPI.ConfigureDriveEdges(trigPin, DriveFormat.NonReturn, driveOn, driveData, driveReturn, driveOff);
                tsMIPI.ConfigureCompareEdgesStrobe(trigPin, compareStrobe);
                // Sclk
                tsMIPI.ConfigureDriveEdges(sclkPins, DriveFormat.NonReturn, driveOn + clockRisingEdgeDelay, driveData + clockRisingEdgeDelay, driveReturn + clockFallingEdgeDelay, driveOff + clockFallingEdgeDelay);
                tsMIPI.ConfigureCompareEdgesStrobe(sclkPins, compareStrobe);
                #endregion
                #endregion
            ...
            public bool LoadVector(List<string> fullPaths, string nameInMemory, bool datalogResults)
            {
                ...
                try
                {
                    ...
                    else if (isVec && notLoaded)
                    {
                        ...
                        // If this is the TempSense or PID vectors, convert to a capture style vector so the read functions can retrive the values appropriately.

                        bool convertToCapture = false;

                        if (!useScript)
                        {
                            convertToCapture = false; //convertToCapture = true; Need to check Seoraksan. 
                        }
                        //  convertToCapture = nameInMemory.Replace("_", "").ToUpper().Contains("TEMPSENSE") || nameInMemory.Replace("_", "").ToUpper().Contains("READTS") || nameInMemory.Replace("_", "").ToUpper().Contains("READPID");
                        ...
                        Timeset timesetWrite = Timeset.MIPI_SCLK_NRZ;
                        Timeset timesetRead = Timeset.MIPI_SCLK_NRZ;
                        if (nameInMemory.Contains("RXOTPVERIFY"))
                        {
                            timesetWrite = Timeset.MIPI_SCLK_NRZ_HALF;
                            timesetRead = Timeset.MIPI_SCLK_NRZ_HALF;
                        }
                         
                        //bool success = this.ConvertVecAndLoadPattern(fullPaths[0], nameInMemory.ToLower(), forceDigiPatRegeneration, Timeset.MIPI, Timeset.MIPI_HALF, ref captureCount, convertToCapture, true);
                        bool success = this.ConvertVecAndLoadPattern(fullPaths[0], nameInMemory.ToLower(), forceDigiPatRegeneration, timesetWrite, timesetRead, ref captureCount, convertToCapture, true);   /////////////migration
                        if (success)
                        {
            ...
            public bool LoadVector(List<string> fullPaths, string nameInMemory, bool datalogResults, bool ReadFlag, bool ReturnCall)                            ////////////////Thaison, Frank
            {
                ...
                try
                {
                    ...
                    bool isDigipat = fullPaths[0].ToUpper().EndsWith(".DIGIPAT");
                    bool isVec = fullPaths[0].ToUpper().EndsWith(".VEC");
                    bool notLoaded = !loadedPatternFiles.Contains(fullPaths[0]);

                    // If this is a digipat file and it hasn't already been loaded into instrument memory, load it
                    if (isDigipat && notLoaded)
                    {
                        DIGI.LoadPattern(fullPaths[0]);
                        loadedPatternFiles.Add(fullPaths[0]);
                        return true;
                    }
                    // If this is a vec file and it hasn't already been loading into instrument memory convert and then load it
                    else if (isVec && notLoaded)
                    {
                        ...
                        //bool success = this.ConvertVecAndLoadPattern(fullPaths[0], nameInMemory.ToLower(), forceDigiPatRegeneration, Timeset.MIPI, Timeset.MIPI_HALF, ref captureCount, convertToCapture, true);
                        bool success = this.ConvertVecAndLoadPattern(fullPaths[0], nameInMemory.ToLower(), forceDigiPatRegeneration, timesetWrite, timesetRead, ref captureCount, ReadFlag, ReturnCall, convertToCapture, true);   /////////////migration
                        if (success)
                        {
                            // Store capture counter so that patterns that were converted to capture can know how much to read back later
                            captureCounters.Add(nameInMemory.Replace("_", "").ToUpper(), captureCount);
            ...
            {
                bool pass = true;
                pass = LoadVector_MipiRegRead() & LoadVector_MipiExtendedRegRead() & LoadVector_MipiExtendedLongRegRead();                          // Read pattern
                pass = pass & LoadVector_MipiRegWrite() & LoadVector_MipiExtendedRegWrite() & LoadVector_MipiExtendedLongRegWrite();                // Write pattern
                return pass & LoadVector_MipiRegWriteRead() & LoadVector_MipiExtendedRegWriteRead() & LoadVector_MipiExtendedLongRegWriteRead();    // Write + read in one pattern
            }
            ...
            /// </summary>
            /// <returns>True if pattern load succeeds.</returns>
            private bool LoadVector_MipiExtendedLongRegWrite()
            {
                if (!usingMIPI)
                {
                    MessageBox.Show("Attempted to load HSDIO MIPI vector but HSDIO.Initialize wasn't called", "HSDIO MIPI", MessageBoxButtons.OK, MessageBoxIcon.Error);
                ...
                try
                {
                    ...
                    foreach (MipiPinNames m in allMipiPinNames)
                    {
                        ...
                        for (int i = 1; i <= 8; i++)
                        {
                            ...
                            // Generate and load Pattern from the formatted array.
                            // Note: Regsiter read/write patterns are generated as Return to Zero Patterns
                            if (!this.GenerateAndLoadPattern("ExtendedLongRegisterWrite" + i.ToString() + "Pair" + p.ToString(), pins, pattern, true, Timeset.MIPI))
                            {
            ...
            /// </summary>
            /// <returns>True if pattern load succeeds.</returns>
            private bool LoadVector_MipiExtendedLongRegRead()
            {
                if (!usingMIPI)
                {
                    MessageBox.Show("Attempted to load HSDIO MIPI vector but HSDIO.Initialize wasn't called", "HSDIO MIPI", MessageBoxButtons.OK, MessageBoxIcon.Error);
                ...
                try
                {
                    ...
                    foreach (MipiPinNames m in allMipiPinNames)
                    {
                        ...
                        for (int i = 1; i <= 8; i++)
                        {
                            ...
                            // Generate and load Pattern from the formatted array.
                            // Note: Regsiter read/write patterns are generated as Return to Zero Patterns
                            if (!this.GenerateAndLoadPattern("ExtendedLongRegisterRead" + i.ToString() + "Pair" + p.ToString(), pins, pattern, true, Timeset.MIPI))
                            {
            ...
            /// </summary>
            /// <returns>True if pattern load succeeds.</returns>
            private bool LoadVector_MipiExtendedLongRegWriteRead()
            {
                if (!usingMIPI)
                {
                    MessageBox.Show("Attempted to load HSDIO MIPI vector but HSDIO.Initialize wasn't called", "HSDIO MIPI", MessageBoxButtons.OK, MessageBoxIcon.Error);
                ...
                try
                {
                    ...
                    foreach (MipiPinNames m in allMipiPinNames)
                    {
                        ...
                        for (int i = 1; i <= 8; i++)
                        {
                            ...
                            // Generate and load Pattern from the formatted array.
                            // Note: Regsiter read/write patterns are generated as Return to Zero Patterns
                            if (!this.GenerateAndLoadPattern("ExtendedLongRegisterWriteRead" + i.ToString() + "Pair" + p.ToString(), pins, pattern, true, Timeset.MIPI))
                            {
                ...
                try
                {
                    ...
                    for (int i = 0; i < wfms.Count; i++)
                    {
                        if (i != 0)
                            returnval += ",";
                        if (i != wfms.Count() - 1)
                            returnval += RegWriteRead_Initiate(pair, slaveAddress_hex, wfms[i], false);
                        else
                            returnval += RegWriteRead_Initiate(pair, slaveAddress_hex, wfms[i], sendTrigger);
                    }
            ...
            public string RegWriteReadAny(int pair, string slaveAddress_hex, string mipiCmd, bool sendTrigger = false)
            {
                ...
                return RegWriteReadAny(pair, slaveAddress_hex, regAddr.ToArray(), data.ToArray(), sendTrigger);
            }
            ...
            public int SendVector_VerifyLNA_OTP(string nameInMemory)
            {
                ...
                try
                {
                    ...
                    Int64[] failureCountTX = sdataPin.GetFailCount();
                    Int64[] failureCountRX = sdataPin_RX.GetFailCount();

                    // uint[][] test = new uint[1][];
                    //  DIGI.CaptureWaveforms.Fetch("0", nameInMemory, -1, TimeSpan.FromSeconds(3), ref test);


                    int Num_OTP_Bit_Fails = (int)failureCountRX[0];
            ...
            public int ReadVector(ref int ReadData, string PowerMode)
            {
                bool MipiFlag = false;
                ...
                // Select pattern to burst
                // Always use all lower case names for start labels due to known issue (some uppercase labels not valid)
                DIGI.PatternControl.StartLabel = PowerMode.ToLower();

                //DIGI.CaptureWaveforms.CreateSerial("SDATA_TX", nameInMemory.ToLower(), 1, BitOrder.MostSignificantBitFirst);
                DIGI.CaptureWaveforms.CreateParallel(string.Format("{0}, {1}", SdataChanName_TX, SdataChanName_RX), PowerMode.ToLower());
                ...
                uint[][] test = new uint[1][];
                DIGI.CaptureWaveforms.Fetch("", PowerMode.ToLower(), 38, TimeSpan.FromSeconds(3), ref test);
                //  DIGI.CaptureWaveforms.Fetch("", nameInMemory.ToLower(), 171, TimeSpan.FromSeconds(3), ref test);

                bool[] passFail = DIGI.PatternControl.GetSitePassFail("");
                Int64[] failureCount = sdataPin.GetFailCount();
                Int64[] failureCount2 = sdataPin_RX.GetFailCount();

                ////////////DIGI.ExportSignal(SignalType.PatternOpcodeEvent, "patternOpcodeEvent0", "PXI_Trig7"); ////////////////////////For Timing Test 161222

                //NumExecErrors = (int)failureCount[0] + (int)failureCount2[0];
                //if (debug) Console.WriteLine("SendVector " + nameInMemory + " Bit Errors: " + NumExecErrors.ToString());

                int DataChanBit = 0;
                if (PowerMode.Contains("RX") || PowerMode.Contains("READ21") || PowerMode.Contains("READD0"))
                {
                    DataChanBit = 1;
                }
                else
                ...
                {
                    int dataBitBin1 = (test[0][i] & DataChanBit) == 0 ? 0 : 1;
                    int dataBitBin2 = (test[0][i + 1] & DataChanBit) == 0 ? 0 : 1;
                    int dataBitBin3 = (test[0][i + 2] & DataChanBit) == 0 ? 0 : 1;
                    int dataBitBin4 = (test[0][i + 3] & DataChanBit) == 0 ? 0 : 1;

                    int dataBitBoth = dataBitBin1 & dataBitBin2 & dataBitBin3 & dataBitBin4;

                    Binary += dataBitBoth;
                }

                ReadData = Convert.ToInt32(Binary, 2);
            ...
            public bool ConvertVecAndLoadPattern(string vecPath, string patternName, bool overwrite, Timeset timeSet_write, Timeset timeSet_read, ref uint captureCount, bool convertToCapture = false, bool load = true)
            {
                ...
                string digipatsrcPath = fileDir + "\\" + patternName + ".digipatsrc";
                string digipatPath = fileDir + "\\" + patternName + ".digipat"; ;
                ...
                digipatsrcFile.WriteLine("//\n");
                digipatsrcFile.WriteLine("file_format_version 1.0;");
                //digipatsrcFile.WriteLine("timeset " + timeSet_write.ToString("g") + ", " + timeSet_read.ToString("g") + ";");           ////////////////////Dummy Pattern 
                if (convertToCapture) { }
                else digipatsrcFile.WriteLine("export " + patternName.ToUpper() + ";");
                ...
                string[] vecpins = currentline.Trim().ToUpper().Split(',');
                ...
                digipatsrcFile.WriteLine("{\n" + patternName.ToUpper() + ":");
                //if (patternName == "DummyPattern")                                                                          ////////////////////Dummy Pattern 
                //    digipatsrcFile.WriteLine("{\n"+patternName+":");
                //else
                //    digipatsrcFile.WriteLine("{");

                // Add Capture opcode if this vec is intended to be converted to a capture style pattern


                //if (convertToCapture)
                //{
                //    // Read the current line from the vec file
                //    currentline = vecFile.ReadLine();
                ...
                //    foreach (string pin in this.allDutPins)
                //    {
                //        if (vecpins.Contains(pin))
                //        {
                //            lineOutput += (lineOutput != "" ? "\t" : "") + lineData[Array.IndexOf(vecpins, pin)];
                //        }
                //        else
                //        {
                //            lineOutput += (lineOutput != "" ? "\tX" : "X");
                //        }
                //    }
                //    // Add extra lines, requires at least 16 x before first capture, duplicate first and add repeat to guarantee we have this much
                //    // digipatsrcFile.WriteLine("repeat(16)\t" + timeSet_write.ToString("g") + "\t" + lineOutput + ";");
                //    // Add capture opcode based on patternName
                //    digipatsrcFile.WriteLine("capture_start(" + patternName + ")\t" + timeSet_write.ToString("g") + "\t0\t0\t1\t0\t0\t1\tX\tX\tX\tX\tX;");
                //}
                ...
                while ((currentline = vecFile.ReadLine()) != null)
                {
                    ...
                    foreach (string pin in this.allDutPins)
                    {
                        if (vecpins.Contains(pin.ToUpper()))//.ToUpper()))
                        {
                            lineOutput += (lineOutput != "" ? "\t" : "") + lineData[Array.IndexOf(vecpins, pin.ToUpper())];//.ToUpper())]; //.ToUpper()
                        }
                        else
                        {
                            lineOutput += (lineOutput != "" ? "\tX" : "X");
                    ...
                    string lastvactor = "";                                                                      ////////////////////Dummy Pattern   
                    // Convert H/L to V if specified
                    // if (convertToCapture & (lineOutput.Contains('H') | lineOutput.Contains('L')))
                    if (convertToCapture & (lineOutput.Contains('H') | lineOutput.Contains('L')))
                    {
                        // Find SCLK Pin location in pins array
                        int TX_sdatapin = Array.IndexOf(allDutPins, SdataChanName_TX);
                        int RX_sdatapin = Array.IndexOf(allDutPins, SdataChanName_RX);

                        // If this is the rising edge of SCLK, add the compare.
                        string[] pinValues = lineOutput.Trim().Split('\t');
                        if (pinValues[TX_sdatapin] == "L" || pinValues[TX_sdatapin] == "H" || pinValues[RX_sdatapin] == "L" || pinValues[RX_sdatapin] == "H")
                        {

                            lineOutput = lineOutput.Replace('H', 'V').Replace('L', 'V');
                            // Write each vector line to the digipatsrc file, add halt opcode on last vector line
                            digipatsrcFile.WriteLine("capture\t" + timeSet_read.ToString("g") + "\t" + lineOutput + ";");
                            ReadCount++;
                        ...
                        // If this is the falling edge of SCLK, don't add the compare.
                        else
                        {
                            lineOutput = lineOutput.Replace('H', 'X').Replace('L', 'X');
                            // Write each vector line to the digipatsrc file, add halt opcode on last vector line
                            digipatsrcFile.WriteLine("\t" + timeSet_read.ToString("g") + "\t" + lineOutput + ";");
                        }
                    }
                    else if (patternName == "txtrigon" || patternName == "rxtrigon" || patternName == "pabiastrig" || patternName.Contains("lna"))
                    {
                        if (patternName == "txtrigon" || patternName == "pabiastrig")
                        {
                            if (Marker_Count == 166)
                            {
                                digipatsrcFile.WriteLine("pulse_signal(event1)\t" + timeSet_read.ToString("g") + "\t" + lineOutput + ";");
                            }
                            else
                            {
                                digipatsrcFile.WriteLine("\t" + timeSet_read.ToString("g") + "\t" + lineOutput + ";");
                        ...
                        else if (patternName.Contains("lna"))
                        {
                            if (Marker_Count == 100)
                            {
                                digipatsrcFile.WriteLine("pulse_signal(event1)\t" + timeSet_read.ToString("g") + "\t" + lineOutput + ";");
                        ...
                        {
                            digipatsrcFile.WriteLine((vecFile.EndOfStream ? lastvactor : "") + "\t" + timeSet_write.ToString("g") + "\t" + lineOutput + ";");
                        }
                    ...
                    else
                    {
                        ...
                        if (convertToCapture)
                        {
                            if (vecFile.EndOfStream)
                            {
                                digipatsrcFile.WriteLine("capture_stop\t" + timeSet_write.ToString("g") + "\t" + lineOutput + ";");
                            }

                            lastvactor = "halt";

                            if ((lineOutput.Contains('H') | lineOutput.Contains('L') | lineOutput.Contains('V')))
                            {

                                digipatsrcFile.WriteLine((vecFile.EndOfStream ? lastvactor : "") + timeSet_read.ToString("g") + "\t" + lineOutput + ";");
                                ReadCount++;
                        ...
                        {
                            lastvactor = "halt";  //lastvactor = "return";need to check seoraksan
            ...
            /// 
            /// 
            public bool ConvertVecAndLoadPattern(string vecPath, string patternName, bool overwrite, Timeset timeSet_write, Timeset timeSet_read, ref uint captureCount, bool ReadFlag, bool ReturnCall, bool convertToCapture = false, bool load = true)
            {
                patternName = patternName.Replace("_", "").ToLower();
                ...
                if (ReadFlag)
                {
                    ...
                    // digipatsrcFile.WriteLine("repeat(16)\t" + timeSet_write.ToString("g") + "\t" + lineOutput + ";");
                    // Add capture opcode based on patternName
                    digipatsrcFile.WriteLine("capture_start(" + patternName + ")\t" + timeSet_write.ToString("g") + "\t0\t0\t1\t0\t0\t1\tX;");
